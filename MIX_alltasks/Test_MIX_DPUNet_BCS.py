...
parser.add_argument('--end_step', type=int, default=200000, help='epoch number of model')
...
parser.add_argument('--task_id', type=int, default=1, help='the task id: BCS-1')
parser.add_argument('--gpu_list', type=str, default='0', help='gpu index')

parser.add_argument('--matrix_dir', type=str, default='../BCS/sampling_matrix', help='sampling matrix directory')
parser.add_argument('--model_dir', type=str, default='model', help='trained or pre-trained model directory')
parser.add_argument('--data_dir', type=str, default='../BCS/data', help='training or test data directory')
parser.add_argument('--log_dir', type=str, default='log', help='log directory')
parser.add_argument('--result_dir', type=str, default='result', help='result directory')
parser.add_argument('--test_name', type=str, default='Set11', help=' or BSD68')
...
model.load_state_dict(torch.load('./%s/net_params_%d.pkl' % (model_dir, end_step)))
...
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        Hypern = torch.FloatTensor([task_id/5, cs_ratio/50,alpha/50]).to(device)  
        x_output = model(Phix, Phi, Qinit, Hypern)

        x_init = torch.mm(Phix, torch.transpose(Qinit, 0, 1))

        end = time()

        Prediction_value = x_output.cpu().data.numpy()
        Init_value = x_init.cpu().data.numpy()


        X_rec = np.clip(col2im_CS_py(Prediction_value.transpose(), row, col, row_new, col_new), 0, 1)
        X_init = np.clip(col2im_CS_py(Init_value.transpose(), row, col, row_new, col_new), 0, 1)
        ...
        print("[%02d/%02d] Run time for %s is %.4f, PSNR is %.2f, SSIM is %.4f" % (img_no, ImgNum, imgName, (end - start), init_PSNR, init_SSIM))

        Img_rec_yuv[:,:,0] = X_rec*255
        Img_init_yuv[:,:,0] = X_init*255

        im_rec_rgb = cv2.cvtColor(Img_rec_yuv, cv2.COLOR_YCrCb2BGR)
        im_rec_rgb = np.clip(im_rec_rgb, 0, 255).astype(np.uint8)

        im_init_rgb = cv2.cvtColor(Img_init_yuv, cv2.COLOR_YCrCb2BGR)
        im_init_rgb = np.clip(im_init_rgb, 0, 255).astype(np.uint8)

        resultName = imgName.replace(args.data_dir, args.result_dir)
        cv2.imwrite("%s_DPUNet_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, end_step, rec_PSNR, rec_SSIM), im_rec_rgb)
        cv2.imwrite("%s_Init_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, end_step, init_PSNR, init_SSIM), im_init_rgb)
        del x_output
...
print('\n')
output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f, Epoch number of model is %d \n" % (cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), end_step)
print(output_data)

output_file_name = "./%s/PSNR_SSIM_Results_CS_DPUNet_plus_layer_%d_group_%d_ratio_%d_lr_%.4f.txt" % (args.log_dir, layer_num, group_num, cs_ratio, learning_rate)

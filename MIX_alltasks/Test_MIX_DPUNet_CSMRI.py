...
parser.add_argument('--task_id', type=int, default=2, help='the task id: CSMRI-2')
parser.add_argument('--gpu_list', type=str, default='0', help='gpu index')

parser.add_argument('--matrix_dir', type=str, default='../CSMRI/sampling_matrix', help='sampling matrix directory')
parser.add_argument('--model_dir', type=str, default='model', help='trained or pre-trained model directory')
parser.add_argument('--data_dir', type=str, default='../CSMRI/data', help='training or test data directory')
...
model.load_state_dict(torch.load('./%s/net_params_200000.pkl' % (model_dir)))
...
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        im_init_rgb = np.clip(X_init*255, 0, 255).astype(np.uint8)
        im_rec_rgb = np.clip(X_rec*255, 0, 255).astype(np.uint8)

        resultName = imgName.replace(args.data_dir, args.result_dir)
        cv2.imwrite("%s_Initial_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, epoch_num, init_PSNR, init_SSIM), im_init_rgb)
        cv2.imwrite("%s_DPUNet_plus_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
        del x_output
...
# output_file_name = "./%s/PSNR_SSIM_Results_MRI_CS_DPUNet_layer_%d_group_%d_ratio_%d.txt" % (args.log_dir, layer_num, group_num, cs_ratio)

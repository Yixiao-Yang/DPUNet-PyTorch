...
class DPUNet(torch.nn.Module):
    ...
    def forward(self, Phix, Phi, Qinit, Hyperparam):
        ...
        x_devec = x.view(-1,1,33,33)
        
        x_rec = col2im_CS_batch_py(x_devec, 99, 99)   # Transfer to the whole patch-based reconstruction
        
        x_deblock = self.deblock(x_rec, Hyperparam)  # Deblocking

        x_final = x_rec + x_deblock

        return [x_final, x_rec]
...
model.load_state_dict(torch.load('./model/BCS_noiseless_FRGM_DPUNet_layer_10_group_1_lr_0.0001/net_params_200.pkl'), strict=False)
...
model_dir = "./%s/BCS_noiseless_FRGM_DPUNet_plus_Unet_dynamic_joint_layer_%d_group_%d_lr_%.4f" % (args.model_dir, layer_num, group_num, learning_rate)

log_file_name = "./%s/Log_BCS_noiseless_FRGM_DPUNet_plus_Unet_dynamic_joint_layer_%d_group_%d_lr_%.4f.txt" % (args.log_dir, layer_num, group_num, learning_rate)
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    for data in rand_loader:
        ...
        # Get block-based data
        block_x = img2col_batch_py(batch_x, 33)
        
        vect_x = block_x.view(-1, 33*33)
        ...
        Phix = torch.mm(vect_x, torch.transpose(Phi, 0, 1))
        ...
        if use_amp:
            with autocast():
                [x_output,x_rec] = model(Phix, Phi, Qinit, Hypern)
        ...
        output_data = "[%02d/%02d] Preconstruction Loss: %.4f, Deblock Loss: %.4f\n" % (epoch_i, end_epoch, loss_pre.item(), loss_deblock.item())
        print(output_data)

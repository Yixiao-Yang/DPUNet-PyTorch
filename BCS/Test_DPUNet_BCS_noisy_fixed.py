...
parser.add_argument('--sampling_ratio', type=int, default=50, help='from {1, 4, 10, 25, 30, 40, 50}')
...
model_dir = "./%s/BCS_noisy_fixed_DPUNet_layer_%d_group_%d_lr_%.4f" % (args.model_dir, layer_num, group_num, learning_rate)
...
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        cv2.imwrite("%s_BCS_noisy_fixed_DPUNet_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
        del x_output
...
output_file_name = "./%s/PSNR_SSIM_Results_BCS_noisy_fixed_DPUNet_layer_%d_group_%d_ratio_%d_lr_%.4f.txt" % (args.log_dir, layer_num, group_num, cs_ratio, learning_rate)
